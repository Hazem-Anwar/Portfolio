...
const skills = [
  { 
    category: "DESIGN", 
    items: ["UI/UX Strategy", "Design Systems Architecture", "Advanced Prototyping", "Information Architecture", "User Research & Testing", "Visual Identity", "Accessibility (WCAG)"] 
  },
  { 
    category: "FRONTEND", 
    items: ["React / Next.js", "TypeScript", "Tailwind CSS", "GSAP / Framer Motion", "WebGL / Three.js", "Performance Optimization", "Git / CI/CD"] 
  }
];
...
export default function AboutModal({ isOpen, onClose }: AboutModalProps) {
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      ...
      if (isMobile) {
        gsap.to(modalRef.current, { y: "100%", duration: 0.45, ease: "power2.in" });
      } else {
        gsap.to(modalRef.current, { scale: 0.96, opacity: 0, y: 40, duration: 0.35, ease: "power2.in" });
      }
  ...
  return (
    <>
      <style jsx global>{`
        ...
        .modal-scrollbar::-webkit-scrollbar-thumb { background: #eee; border-radius: 10px; }
      `}</style>
      ...
      >
        ...
        >
          ...
          <div className="px-6 md:px-10 py-6 md:py-8 border-b border-[#f0f0f0] flex flex-row items-start justify-between gap-4 bg-white z-10 shrink-0 relative animate-el">
             <div className="flex-1">
                <div className="flex items-center gap-3 mb-1.5 flex-wrap">
                   <h1 className="font-bricolage font-extrabold text-[20px] md:text-[26px] text-[#111] leading-none tracking-tight shrink-0">Hazem Anwar<span className="text-[#ff4d00]">.</span></h1>
                   <div className="flex md:hidden items-center translate-y-[-1px] shrink-0">
                      <Image src="/images/about/verified.png" alt="Verified" width={18} height={18} className="object-contain" />
                   </div>
                </div>
                <div className="flex items-center gap-2">
                   ...
                   <div className="hidden md:flex items-center translate-y-[-1px] shrink-0">
                      <Image src="/images/about/verified.png" alt="Verified" width={22} height={22} className="object-contain" />
                   </div>
                </div>
             </div>
             
             <button onClick={onClose} className="w-8 h-8 md:w-9 md:h-9 bg-[#f5f5f5] hover:bg-[#111] hover:text-white rounded-full flex items-center justify-center transition-all shrink-0">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="w-[14px] h-[14px] md:w-[18px] md:h-[18px]"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
             </button>
          </div>

          {/* SCROLLABLE BODY — LIGHT MODE */}
          <div className="flex-1 overflow-y-auto modal-scrollbar p-8 md:p-10 flex flex-col gap-10 bg-white text-[#111]" data-lenis-prevent>
             
             {/* 01: SUMMARY */}
             <section className="animate-el">
                <span className="block font-mono text-[10px] font-bold text-[#2c2c2c] uppercase tracking-[0.1em] mb-4">Summary</span>
                <p className="text-[14px] md:text-[15.5px] text-[#111] leading-relaxed font-medium max-w-[580px]">
                   I specialize in bridging the gap between design and production. By focusing on scalable systems and interface logic, I build products that function as beautifully as they look 🚀.
                </p>
             </section>
             ...
             <div className="flex flex-col gap-6">
                <div className="flex items-center justify-between pb-3 border-b border-[#f0f0f0] animate-el">
                   <span className="font-mono text-[10px] font-bold text-[#2c2c2c] uppercase tracking-[0.1em]">Experience</span>
                   <span className="font-mono text-[8px] text-[#ccc] hidden md:block select-none">/ 02</span>
                </div>
                
                <div className="flex flex-col">
                   {timelineData.map((item, i) => (
                     <div key={i} className="group py-6 border-b border-[#f5f5f5] last:border-0 -mx-6 px-6 md:-mx-10 md:px-10 hover:bg-[#fafafa] transition-colors rounded-xl animate-el">
             ...
             <div className="flex flex-col gap-6 pb-8">
                <div className="flex items-center justify-between pb-3 border-b border-[#f0f0f0] animate-el">
                   <span className="font-mono text-[10px] font-bold text-[#2c2c2c] uppercase tracking-[0.1em]">Expertise</span>
                   <span className="font-mono text-[8px] text-[#ccc] hidden md:block select-none">/ 03</span>
                </div>
                   ...
                   {skills.map((s, i) => (
                     <div key={i} className="flex flex-col gap-4 animate-el">
                        ...
                        <div className="flex flex-wrap gap-2">
                           {s.items.map((item, j) => (
                             <span key={j} className="px-3 py-1 bg-[#f8f8f8] text-[#777] text-[11px] font-bold rounded-md border border-[#eee] hover:border-[#111] hover:bg-white hover:text-[#111] transition-all duration-300 cursor-default">
                                {item}
                             </span>
                           ))}
                               ...
                               <div className="fixed md:absolute bottom-6 md:bottom-[calc(100%+12px)] left-1/2 -translate-x-1/2 px-5 py-4 bg-[#111] text-white rounded-2xl shadow-2xl opacity-0 invisible translate-y-2 group-hover/counter:opacity-100 group-hover/counter:visible group-hover/counter:translate-y-0 group-active/counter:opacity-100 group-active/counter:visible group-active/counter:translate-y-0 transition-all duration-500 z-[300] w-[min(320px,85vw)] pointer-events-none border border-white/10">
                                 <div className="flex flex-col gap-3">
                                    ...
                                    <div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
                                       {(s.category === "DESIGN" 
                                          ? ["Figma Adv.", "Adobe PS", "Adobe AI", "Principle", "Lottie", "Color Theory", "Typography", "User Testing", "A/B Testing", "Aesthetic Str.", "Wireframing", "Handoff Logic", "Style Guides", "Micro-int.", "Design Psych."]
                                          : ["Redux", "Zustand", "Storybook", "Unit Testing", "E2E Testing", "Vite", "TurboRepo", "Web Vitals", "Sanity CMS", "Headless UI", "Shadcn UI", "Framer Motion", "Three.js", "GLSL", "APIs", "Axios", "GraphQL", "SWR", "React Query", "Hooks", "PWA", "SEO Core", "A11y", "Audit"]
                                       ).map((h, k) => (
                                          <div key={k} className="flex items-center gap-2">
                                             <div className="w-1 h-1 rounded-full bg-white/20"></div>
                                             <span className="text-[10px] font-bold text-white/70 whitespace-nowrap">{h}</span>
                                          </div>
                                       ))}
                                    </div>
                                 </div>
          ...
          <div className="px-8 md:px-10 py-5 md:py-6 border-t border-[#f0f0f0] flex flex-col sm:flex-row items-center justify-between bg-[#f9f9f9] shrink-0 z-10 gap-4 sm:gap-0 animate-el">
             <a href="/resume.pdf" target="_blank" className="group relative flex items-center gap-3 bg-white hover:bg-[#111] hover:text-white text-[#111] px-4 py-2 rounded-full transition-all duration-300 shadow-sm border border-[#eee]">
                <div className="w-6 h-6 bg-[#ff4d00] rounded-[4px] flex items-center justify-center shrink-0 transition-colors">
                   <span className="text-white text-[10px] font-black tracking-tighter leading-none">PDF</span>
                </div>
